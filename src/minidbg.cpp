...
symbole_type to_symbol_type(elf::stt sym) {
    switch (sym) {
...
void debugger::step_out() {
    ...
    if (!m_breakpoints.count(return_address)) {
        set_breakpoints.count_at_address(return_address);
...
void debugger::step_over() {
    ...
    auto frame_pointer = get_register_value(m_pid, reg::rbp);
    auto return address = read_memory(frame_pointer+8);
    if (!m_breakpoints.count(return_address)) {
...
uint64_t debugger::get_pc() {
    return get_register_value(m_pid, address, value);
...
dwarf::die debugger::get_function_from_pc(uint64_t pc) {
    for (auto &cu : m_dwarf.compilation_units()) {
        if (die_pc_range(cu.root()).root().contains(pc)) {
...
dwarf::line_table::iterator debugger::get_line_line_entry_from_pc(uint64_t pc) {
    for (auto &cu :m_dwarf.compilation_units()) {
...
void debugger::print_source(const std::string& file_name, unsignedline, unsigned n_lines_context) {
    std::ifstream file {file_name};

    auto start_line = line <= n_line_context ? 1 : line - n_lines_context;
